...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.return_local_features:
            ...
            x_local = F.linear(x_local, self.c_proj.weight, self.c_proj.bias)    # x_local=proj(v(x))
        ...
        x2, _ = F.multi_head_attention_forward(
        ...
        x_global = x1[:1].squeeze(0)
        x_regional = x2.permute(1, 0, 2)
        x_local = x_local.reshape(b, h * w, -1)
        
        return x_global, x_regional, x_local
...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.layer3(x)
        x = self.layer4(x)
        x_global, x_regional, x_local= self.attnpool(x)
        return x_global, x_regional, x_local
...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        v2 = x[1:, :, :] + self.attention(self.ln_1(x[1:, :, :]))
        v2 = v2 + self.mlp(self.ln_2(v2))
...
class CLIP(nn.Module):
    ...
    def encode_text(self, text: torch.Tensor, batch_first: bool = False) -> torch.Tensor:
        ...
        x, q, k, v, v2 = self.transformer(x, batch_first=batch_first)
