...
class NWPU_RESISC45Dataset(Dataset):
    ...
    def __init__(
        ...
        db = lib.load_json(os.path.join(abs_path_dirname, 'files', 'split_nwpu_resisc45.json'))
        db = db[split]
        ...
        for x in db:
            self.paths.append(
                os.path.join(self.root, 'image', x[0])
            )
